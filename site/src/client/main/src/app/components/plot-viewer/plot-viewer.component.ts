  ...
  refresh(fullModel: object): void {
    ...
    if (fullModel && !this.waiting) {
      ...
      try {
        ...
        if (window.location.pathname.indexOf('/tools/compare') > -1 || window.location.pathname.indexOf('/tools/report') > -1) {
          setTimeout(() => {
        ...
        chart.dispose();
        const errorInfo = {
          type: (fullModel as any).renderMethod,
          content: 'ErrorInfo ' + e.message
        };
        this.contentChanged.emit(errorInfo);
      }
